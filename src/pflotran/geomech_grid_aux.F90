...
function GMGridCreate()
  ...
  nullify(geomech_grid%node_ids_local_natural)
  nullify(geomech_grid%ghosted_node_ids_natural)
  nullify(geomech_grid%ghosted_node_ids_petsc)
  nullify(geomech_grid%nL2G)
  nullify(geomech_grid%nG2L)
  nullify(geomech_grid%nG2A)
  nullify(geomech_grid%gauss_node)
  nullify(geomech_grid%gauss_surf_node)
  geomech_grid%mapping_num_cells = 0
  nullify(geomech_grid%mapping_cell_ids_flow)
  nullify(geomech_grid%mapping_vertex_ids_geomech)
...
subroutine GMVecSetTypeFromDM(vec,dm)
  !
...
end subroutine GMVecSetTypeFromDM
...
!
! ************************************************************************** !
subroutine GMCreateGMDM(geomech_grid,dm_ptr,ndof,option)
  ...
  call GMVecSetTypeFromDM(gmdm%global_vec,dm_ptr%dm)
  ...
  call GMVecSetTypeFromDM(gmdm%local_vec,dm_ptr%dm)
...
!
! ************************************************************************** !
subroutine GMGridDMCreateVector(geomech_grid,dm_ptr,vec,vec_type,option)
  ...
  select case(vec_type)
    case(GLOBAL)
      ...
      call GMVecSetTypeFromDM(vec,dm_ptr%dm)
...
!
! ************************************************************************** !
subroutine GMGridDMCreateVectorElem(geomech_grid,dm_ptr,vec,vec_type,option)
...
subroutine GMGridDestroy(geomech_grid)
  ...
  call DeallocateArray(geomech_grid%nG2L)
  call DeallocateArray(geomech_grid%nL2G)
  call DeallocateArray(geomech_grid%nG2A)
  call DeallocateArray(geomech_grid%mapping_cell_ids_flow)
  call DeallocateArray(geomech_grid%mapping_vertex_ids_geomech)
